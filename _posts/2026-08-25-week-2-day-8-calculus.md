...
And crucially, this has to work for _every_ $\varepsilon$ — shrink the
...
That gives a clean way to say a limit _doesn't_ exist: no matter how small
...
$a$ — that's what a derivative _is_, the local slope. So near $a$:
...
they're shrinking at _rates_ given by their derivatives, and it's that ratio
of rates — not the vanishing values themselves — that the limit actually
measures.

## Integration: area as accumulated output

Switch questions entirely. Instead of _rates of change_, integration asks
about _accumulation_.
...
under the curve _is_ that sum, in the limit as the slivers shrink to zero
width.
...
moving backward, and moving backward should _subtract_ from net distance
